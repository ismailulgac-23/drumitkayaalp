...
export default function Marquee2Items() {
    ...
    const fetchItems = async () => {
        try {
            setLoading(true);
            const response = await axios.get('/marquee2-items');
    ...
    const handleDelete = async (id) => {
        ...
        try {
            await axios.delete(`/marquee2-items/${id}`);
    ...
    return (
        <div>
            <PageBreadcrumb pageTitle="Marquee2 Items" />
            <div className="space-y-6">
                <ComponentCard 
                    title="Marquee2 Items Yönetimi"
                    titleRightRenderer={
                        <Link href="/site-content/marquee2-items/create">
                        ...
                        renderItem={(item) => (
                            <TableRow key={item.id}>
                                ...
                                <TableCell className="px-5 py-4 sm:px-6 text-start flex gap-2">
                                    <Link href={`/site-content/marquee2-items/edit/${item.id}`}>
